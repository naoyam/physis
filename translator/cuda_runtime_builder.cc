...
void CUDARuntimeBuilder::BuildRunKernelFuncBody(
    StencilMap *stencil, SgFunctionParameterList *param,
    vector<SgVariableDeclaration*> &indices,
    SgBasicBlock *body) {
  LOG_DEBUG() << __FUNCTION__ << "\n";
  si::attachComment(body, "Generated by " + string(__FUNCTION__));
  
  Builder()->BuildKernelIndices(stencil, body, indices);

  SgScopeStatement *kernel_call_block = 
      BuildKernelCallPreamble(stencil, param, indices, body);
...
SgFunctionDeclaration *CUDARuntimeBuilder::BuildRunKernelFunc(StencilMap *stencil) {
  ...
  vector<SgVariableDeclaration*> indices;
  BuildRunKernelFuncBody(stencil, params, indices,
                         run_func->get_definition()->get_body());
