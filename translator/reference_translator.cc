...
SgFunctionCallExp *
ReferenceTranslator::generateKernelCall(StencilMap *s,
...
SgBasicBlock* ReferenceTranslator::generateRunKernelBody(
  ...
  SgFunctionCallExp *kernelCall = generateKernelCall(s, indexArgs,
                                                     innerMostBlock);
...
SgFunctionDeclaration *ReferenceTranslator::generateRunKernel(StencilMap *s) {
  SgFunctionParameterList *parlist = sb::buildFunctionParameterList();
  ...
  runFunc->get_definition()->set_body(generateRunKernelBody(s, stencil_param));
  return runFunc;
}

SgBasicBlock *ReferenceTranslator::generateRunBody(Run *run) {
  SgBasicBlock *block = sb::buildBasicBlock();
...
SgFunctionDeclaration *ReferenceTranslator::generateRun(Run *run) {
  ...
  fdef->set_body(generateRunBody(run));
