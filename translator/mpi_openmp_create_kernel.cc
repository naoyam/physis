...
SgBasicBlock* MPIOpenMPTranslator::BuildRunKernelBody(
  ...
  // for (int unit = 0; unit < division_[0] * division_[1] * division_[2]; unit++) {
  //   int ii = unit % division_[0];
  //   int jj = (unit / division_[0]) % division_[1];
  //   int kk = (unit / division_[0] / division_[1]);
  //   
  //   int i_width = (dom.local_max[0] - dom.local_min[0]);
  //   i_width = (i_width + division_[0] - 1) / division_[0];
  //   int i_start = dom.local_min[0] + i_width * ii;
  //   int i_end = min(dom.local_max[0], i_start + i_width);
  //   
  //   :
  //   
...

  {
    ...
    
    {
      ...
      si::appendStatement(sb::buildPragmaDeclaration("omp parallel for"), loopBlock);
    ...
    for (int i = maxdim - 1; i >= 0; i--) {
      ...
      SgVariableDeclaration * i_indexDecl =
        sb::buildVariableDeclaration(
            getLoopIndexName(i),
            sb::buildUnsignedLongType(),
            NULL, loopBlock);
      indexArgs.insert(indexArgs.begin(), sb::buildVarRefExp(i_indexDecl));
      si::appendStatement(i_indexDecl, loopBlock);
    
      SgStatement *init =
        sb::buildAssignStatement(
            sb::buildVarRefExp(i_indexDecl),
            si::copyExpression(indStartExpression));
      SgStatement *test =
        sb::buildExprStatement(
            sb::buildLessThanOp(
                sb::buildVarRefExp(i_indexDecl),
                si::copyExpression(indEndExpression)));
      SgExpression *incr = sb::buildPlusPlusOp(sb::buildVarRefExp(i_indexDecl));

      SgBasicBlock *inner_innerBlock = sb::buildBasicBlock();
      SgStatement *loopStatement = sb::buildForStatement(init, test, incr, inner_innerBlock);
      si::appendStatement(loopStatement, loopBlock);
