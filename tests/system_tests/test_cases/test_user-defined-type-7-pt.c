/*
 * TEST: Run 7-pt stencil on a user-defined type
 * DIM: 3
 * PRIORITY: 1
 */
...
void kernel1(const int x, const int y, const int z,
             PSGrid3DPoint g) {
  float v = PSGridGet(g, x, y, z).p + PSGridGet(g, x+1, y, z).p +
      PSGridGet(g, x-1, y, z).p + PSGridGet(g, x, y+1, z).p +
      PSGridGet(g, x, y-1, z).p + PSGridGet(g, x, y, z+1).p +
