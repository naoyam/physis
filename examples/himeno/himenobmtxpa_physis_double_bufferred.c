...
void mat_set(PSGrid3DFloat Mat, float z);
void mat_set_init(PSGrid3DFloat Mat);
...
main(int argc, char *argv[])
{
  ...
  // Would be better if arrays could be used.
...
void
mat_set(PSGrid3DFloat mat, float val)
{
  int i,j,k;
  ...
  for(i=0; i< PSGridDim(mat, 0); i++)
    for(j=0; j< PSGridDim(mat, 1); j++)
      for(k=0; k< PSGridDim(mat, 2); k++)
        PSGridSet(mat, i, j, k, val);
}

void
mat_set_init(PSGrid3DFloat Mat)
{
  int  i,j,k;
  ...
  for(i=0; i< PSGridDim(Mat, 0); i++)
    for(j=0; j< PSGridDim(Mat, 1); j++)
      for(k=0; k< PSGridDim(Mat, 2); k++) {
        float v = ((float)(i*i)) /
                  ((PSGridDim(Mat, 0) - 1) * (PSGridDim(Mat, 0) - 1));
        PSGridSet(Mat, i, j, k, v);
      }

}

void jacobi_kernel(const int i, const int j, const int k,
                   PSGrid3DFloat p0, 
       ...
       PSGrid3DFloat bnd, PSGrid3DFloat wrk1)
{
  ...
  PSStencilRun(PSStencilMap(jacobi_kernel, innerDom,
                            p0,  a0, a1, a2, a3, b0, b1, b2,
                            c0, c1, c2, bnd, wrk1, omega), nn);
